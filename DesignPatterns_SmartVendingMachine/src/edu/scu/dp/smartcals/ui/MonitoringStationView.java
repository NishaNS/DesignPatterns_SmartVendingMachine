...
public class MonitoringStationView extends javax.swing.JPanel implements
	...
	private void loadVendingMachinesRadioPanel() {
		...
		for (VendingMachine vm : vendingMachines) {
			...
			String vmText = vm.getLocationType() + "@" + vm.getLocation() + "("
					+ vm.getVendingMachineId() + ")";
			addToVendingMachineRadioGroupPanel(vmText, vm.getVendingMachineId()
					+ "");
		}
	}

	private void btnSearchInventoryActionPerformed(
		...
		if (!txtInvenProdID.getText().isEmpty()) {
			...
			txtInvenPrice
					.setText(Double.toString(invProduct.getProductPrice()));
			txtQuantity.setText(Integer.toString(invProduct.getqty()));
			txtVendingMachineID.setText(Long.toString(invProduct
					.getVendingMachineId()));
	...
	private void btnUpdateInventoryActionPerformed(
			java.awt.event.ActionEvent evt) {
	...
	private void btnDeleteInventoryActionPerformed(
			java.awt.event.ActionEvent evt) {
		if (!txtInvenProdID.getText().isEmpty()) {
			long prodId = Long.parseLong(txtInvenProdID.getText());
	...
	private void btnAddInventoryActionPerformed(java.awt.event.ActionEvent evt) {
		if ((!txtInvenProdID.getText().isEmpty())
				&& (!txtInvenPrice.getText().isEmpty())
				&& (!txtVendingMachineID.getText().isEmpty())
				&& (!txtQuantity.getText().isEmpty())) {
			int prodId = Integer.parseInt(txtInvenProdID.getText());
			double price = Double.parseDouble(txtInvenPrice.getText());
			int vendMachId = Integer.parseInt(txtVendingMachineID.getText());
			int qty = Integer.parseInt(txtQuantity.getText());
			boolean result = admin.addInventoryData(prodId, price, vendMachId,
					qty);
			if (result) {
	...
	class VMRadioButtonActionListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (actionCommand.equals("ALL")) {
				...
				try {
					tblRevenue.setModel(revenueTableController.getModel()
							.createAndFetchModelData());
			...
			if (actionCommand.equals("SEARCH_PRODUCT")) {
				if (!txtProductID.getText().isEmpty()) {
					...
					try {
						...
						txtProductCategory.setText(productModel.getCategory()
								.toString());
						txtProductName.setText(productModel.getProductName());
						txtProductPrice.setText(Double.toString(productModel
								.getProductPrice()));
			...
			if (actionCommand.equals("DELETE_PRODUCT")) {
				if (!txtProductID.getText().isEmpty()) {
					...
					try {
						...
						e1.printStackTrace();
						JOptionPane.showMessageDialog(null,
								"Unable to delete product id " + productId);
						return;
					}
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (validateData()) {
				...
				if (actionCommand.equals("SEARCH_NUTRI")) {
					...
					try {
						String allNutriValues = admin.searchNutriInfo(
								Long.parseLong(txtNutriProdID.getText()))
								.formatData();
						String[] splitNutriValues = allNutriValues.trim()
								.split(", ");
						for (String element : splitNutriValues) {
							if (element.startsWith("Product")) {
								int start = element.indexOf(":");
								txtNutriProdID.setText(element.substring(
										start + 2, element.length()));
							} else if (element.startsWith("Serving")) {
								int start = element.indexOf(":");
								txtServingSize.setText(element.substring(
										start + 2, element.length()));
							} else if (element.startsWith("Calorie")) {
								int start = element.indexOf(":");
								txtCalories.setText(element.substring(
										start + 2, element.length()));
							} else if (element.startsWith("Total Fat")) {
								int start = element.indexOf(":");
								txtTotalFat.setText(element.substring(
										start + 2, element.length()));
							} else if (element.startsWith("Saturated Fat")) {
								int start = element.indexOf(":");
								txtSaturatedFat.setText(element.substring(
										start + 2, element.length()));
							} else if (element.startsWith("Trans Fat")) {
								int start = element.indexOf(":");
								txtTransFat.setText(element.substring(
										start + 2, element.length()));
							} else if (element.startsWith("Cholestrol")) {
								int start = element.indexOf(":");
								txtCholestrol.setText(element.substring(
										start + 2, element.length()));
							} else if (element.startsWith("Sodium")) {
								int start = element.indexOf(":");
								txtSodium.setText(element.substring(start + 2,
										element.length()));
							} else if (element.startsWith("Total Carbs")) {
								int start = element.indexOf(":");
								txtTotalCarbs.setText(element.substring(
										start + 2, element.length()));
							} else if (element.startsWith("Dietary Fiber")) {
								int start = element.indexOf(":");
								txtDietaryFiber.setText(element.substring(
										start + 2, element.length()));
							} else if (element.startsWith("Sugar")) {
								int start = element.indexOf(":");
								txtSugars.setText(element.substring(start + 2,
										element.length()));
							} else if (element.startsWith("Protein")) {
								int start = element.indexOf(":");
								txtProtein.setText(element.substring(start + 2,
										element.length()));
							} else if (element.startsWith("Iron")) {
								int start = element.indexOf(":");
								txtIron.setText(element.substring(start + 2,
										element.length()));
							} else if (element.startsWith("Smart Tag")) {
								int start = element.indexOf(":");
								txtSmartTag.setText(element.substring(
										start + 2, element.length()));
							}
